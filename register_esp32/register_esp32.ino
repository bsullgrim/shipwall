...
// The sprite arrays are RGB565 generated by the Python pipeline in the opposite
// byte order from what Protomatter's drawPixel expects, which swaps red<->blue
// and washes mid-tones. Swapping the two bytes per word at draw time fixes the
// color without regenerating any sprites. If colors ever look wrong again after
// a pipeline change, this is the first thing to check (remove the swap, or add
// it, depending on which order the pipeline now writes).
static inline uint16_t spx(uint16_t c) { return __builtin_bswap16(c); }
...
void drawSpriteScaled(const uint16_t* spr, int ox, int oy, int dim) {
  for (int y = 0; y < dim; y++) {
    ...
    for (int x = 0; x < dim; x++) {
      ...
      if (c != 0x0000) matrix.drawPixel(ox + x, oy + y, spx(c));   // black = transparent
    }
  }
}
...
void drawMini(const uint16_t* mini, int ox, int oy) {
  for (int y = 0; y < MINI_SIZE; y++)
    for (int x = 0; x < MINI_SIZE; x++) {
      ...
      if (c != 0x0000) matrix.drawPixel(ox + x, oy + y, spx(c));
    }
}
...
void labelValue(const char* label, const String& val, int& y) {
  ...
  // Value column runs x=92 to the right edge (128): ~36px = 6 chars at size 1.
  // Clip so a long value can't wrap into the funnel area on the left.
  String v = val;
  if (v.length() > 6) v = v.substring(0, 6);
  textAt(v.c_str(), 92, y, C_VALUE);
  y += 9;
}

// AIS nav-status strings are long ("Under way using engine"); the detail card's
// value column is only ~36px (6 chars). Map to short codes that fit.
String navAbbr(const String& s) {
  String t = s; t.toUpperCase();
  if (t.indexOf("UNDER WAY") >= 0)  return "UWAY";   // under way using engine / sailing
  if (t.indexOf("MOOR") >= 0)       return "MOOR";
  if (t.indexOf("ANCHOR") >= 0)     return "ANCH";
  if (t.indexOf("AGROUND") >= 0)    return "GRND";
  if (t.indexOf("FISH") >= 0)       return "FISH";
  if (t.indexOf("RESTRICT") >= 0)   return "RSTR";   // restricted manoeuvrability
  if (t.indexOf("CONSTRAIN") >= 0)  return "DRFT";   // constrained by draught
  if (t.indexOf("NOT UNDER COMMAND") >= 0) return "NUC";
  if (t.length() > 6) return t.substring(0, 6);      // unknown: clip to fit
  return t;
}
...
void drawDetail(int idx) {
  ...
  { String line2;
    ...
    if (s.navstat.length()) { if (line2.length()) line2 += "  "; line2 += navAbbr(s.navstat); }
